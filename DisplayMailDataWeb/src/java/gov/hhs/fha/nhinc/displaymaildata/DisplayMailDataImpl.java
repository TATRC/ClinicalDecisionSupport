...
public class DisplayMailDataImpl {
    public static final String ZIMBRA_URL = "https://192.168.5.11/";
    ...
    private void sendMail(String userId, String login, String pwd) {
        ...
        try {
            ...
            sslStore.connect(host, login, pwd);
            Transport transport = session.getTransport();
            MimeMessage message = new MimeMessage(session);
            message.setRecipients(Message.RecipientType.TO, "janemdoe@lab.socraticgrid.org");
            message.setSubject("TESTING FROM NETBEANS");
    ...
    private List<SummaryData> retrieveMail(String userType, String userId, String login, String pswd, boolean onlyNew)
        ...
        try {
            ...
            //Get the sslStore
            // sslStore = session.getStore(protocol);
            ...
            //Get folder
            folder = sslStore.getFolder("INBOX");
    ...
    public SetMessageResponseType setMessage(SetMessageRequestType request) {
        sendMail(request.getUserId(), request.getUserId(), "test");
    ...
    public GetMessagesResponseType getMessages(GetMessagesRequestType request) {
        ...
        try {
            List<SummaryData> allMail = retrieveMail("User", "janemdoe@lab.socraticgrid.org",
                    "janemdoe@lab.socraticgrid.org", "test", false);
            for (SummaryData summary : allMail) {
  //             String patientId = summary.getPatient();
  //              if (patientId != null && patientId.equals(request.getPatientId())) {
                    ...
                    DetailData detail = retrieveMailMessage("janemdoe@lab.socraticgrid.org", "test", summary.getItemId());
                    // resp.setFrom(detail.getFrom());
                    resp.setDescription(detail.getDescription());
                    resp.setMessageId(parseMsgId(detail.getItemId())[2]);
                    resp.setMessageDate(detail.getDateCreated());
                    resp.setTitle(detail.getAuthor());
