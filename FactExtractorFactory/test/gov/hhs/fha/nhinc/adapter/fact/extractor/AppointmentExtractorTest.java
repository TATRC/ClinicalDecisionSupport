 ...
 * @author kim
 */
public class AppointmentExtractorTest {

  private String[] testXmlFiles = {
    "test/data/static/outpatient-encounters-single.xml",
    "test/data/static/outpatient-encounters-multiple.xml"
  };
  private int[] factsCount = {1, 2};
  private String[] factsPatientId = {"11428", "11428", "11428"};
  private FactExtractor extractor = null;

  public AppointmentExtractorTest() {
  }
  ...
  public void testAppointmentExtractor() {
    System.out.println("testAppointmentExtractor");
    int i = 0;

    try {
      for (String xmlFile : testXmlFiles) {
        List<Object> facts = runAppointmentExtractor(xmlFile);
  ...
  private void checkFacts(int testIndex, List facts) {
    AppointmentFactType apptFact = null;

    assertEquals(factsCount[testIndex], facts.size());

    for (Object fact : facts.toArray()) {
      apptFact = (AppointmentFactType) fact;
      assertEquals(factsPatientId[testIndex], apptFact.getPatientId().getValue());
    }
  }

  private List<Object> runAppointmentExtractor(String testfile) throws Exception {
    String extractorType = "OutpatientEncounter";
