...
public class DisplayDataAggregatorImpl {
    ...
    public GetMessageDetailResponseType getMessageDetail(GetMessageDetailRequestType request) {
        ...
        response.getMessageDetail().add("STUB");
        return response;
    }
    ...
    public GetMessagesResponseType getMessages(GetMessagesRequestType request) throws Exception {
        ...
        if (messageType.equalsIgnoreCase("All")) {
            ...
            urlMap.put("Email", checkDataSource("Email"));
            urlMap.put("Alerts", checkDataSource("Alerts"));
            urlMap.put("Documents", checkDataSource("Documents"));
            urlMap.put("Archive", checkDataSource("Archive"));
            response = handler.getAllMessages(urlMap, request);
        } else if (messageType.equalsIgnoreCase("Email")) {
            String url = checkDataSource(messageType);
            response = handler.getEmailMessages(url, request);
        } else if (messageType.equalsIgnoreCase("Alerts")) {
            String url = checkDataSource(messageType);
            response = handler.getAlertMessages(url, request);
        } else if (messageType.equalsIgnoreCase("Documents")) {
            String url = checkDataSource(messageType);
            response = handler.getDocumentMessages(url, request);
        } else if (messageType.equalsIgnoreCase("Archive")) {
            String url = checkDataSource(messageType);
            response = handler.getArchiveMessages(url, request);
        }
    ...
    public SetMessageDataResponseType setMessage(SetMessageDataRequestType request) {
        ...
        try {
            ...
            //Call destination
            if (request.getDataSource().equals("Alerts")) {
                response = setAlertMessage(dataSourceUrl, request, response);
            } else if (request.getDataSource().equals("Email")) {
                response = setEmailMessage(dataSourceUrl, request, response);
            }


        } catch (Exception e) {
            String errorMsg = "Error setting message in component data.";
            log.error(errorMsg, e);
            response.setMessage(errorMsg + ". " + e.getMessage());
            response.setSuccessStatus(false);
        }
        return response;
    }

    private SetMessageDataResponseType setAlertMessage(String dataSourceUrl, SetMessageDataRequestType request, SetMessageDataResponseType response) {
        ...
        SetMessageResponseType componentResponse = port.setMessage(componentRequest);
        log.debug("call to display data alert2" + dataSourceUrl);
        response.setMessage(componentResponse.getMessage());
        response.setSuccessStatus(componentResponse.isSuccessStatus());
        return response;
    }

    private SetMessageDataResponseType setEmailMessage(String dataSourceUrl,
            SetMessageDataRequestType request, SetMessageDataResponseType response) {
        ...
        SetMessageResponseType portResponse = port.setMessage(setRequest);
        response.setMessage(portResponse.getMessage());
    ...
    private SetMessageRequestType setMessageComponentRequest(SetMessageDataRequestType request) {
        ...
        componentRequest.setStatus(request.getStatus());
        ...
        componentRequest.setContactBCC(request.getContactBCC());
        componentRequest.setContactCC(request.getContactCC());
        componentRequest.setContactTo(request.getContactTo());
        componentRequest.setDocument(request.getAttachment());
        componentRequest.getFolder().addAll(request.getFolder());
